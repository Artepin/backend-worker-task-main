...
class SharedState:
    ...
    def __init__(self):
        self.items = []
        self.lock = asyncio.Lock()                      # чтобы корутины выполнялись в правильном порядке, необходимо воспользоваться asyncio.lock
    # модификация состояния сервера
    async def modify(self, value: int):
        async with self.lock:                           #теперь, как только в сервер попадает запрос, мы гарантируем, что только одно выполнение корутины имеет доступ к items в один момент времени.
